...
function CasoCard({ caso, onUpdate }: { caso: Caso; onUpdate: () => void }) {
  ...
  const [texto, setTexto] = useState(''); const [autor, setAutor] = useState('')
  const estadoAdmin = caso.estado_admin || 'Pendiente'
  ...
  async function updateEstado(nuevoEstadoAdmin: string) { const global = calcularEstadoGlobal(caso.area, nuevoEstadoAdmin, caso.estado_talent, caso.estado_cx); await supabase.from('casos').update({ estado_admin: nuevoEstadoAdmin, estado: global }).eq('id', caso.id); onUpdate() }
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', opacity: isCerrado ? 0.7 : 1, borderLeft: `4px solid ${bg}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div><SectorBadge label="Admin" estado={estadoAdmin} />{caso.area === 'Admin+Talent' && <SectorBadge label="Talent" estado={caso.estado_talent || 'Pendiente'} />}</div>
      </div>
...
export default function AdminPage() {
  ...
  async function loadCasos() { const { data } = await supabase.from('casos').select('*').order('created_at', { ascending: false }); if (data) setCasos(data.filter(c => c.area === 'Admin' || c.area === 'Admin+Talent')); setLoading(false) }
  useEffect(() => { loadCasos() }, [])
  return <div style={{ padding: 24 }}><h1 style={{ fontSize: 22, fontWeight: 700, color: '#264534', marginBottom: 16 }}>Admin</h1>{loading ? <p>Cargando...</p> : [...casos].sort((a,b) => ordenEstado(a.estado)-ordenEstado(b.estado)).map(c => <CasoCard key={c.id} caso={c} onUpdate={loadCasos} />)}</div>
}
