...
export default function TalentPage() {
  ...
  async function updateEstado(caso: Caso, nuevoEstadoTalent: string) {
    const nuevoGlobal = calcularEstadoGlobal(caso.area, caso.estado_admin, nuevoEstadoTalent, caso.estado_cx)
    await supabase.from('casos').update({ estado_talent: nuevoEstadoTalent, estado: nuevoGlobal }).eq('id', caso.id)
    loadCasos()
  }
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {loading ? <p>Cargando...</p> : sorted.length === 0 ? <p style={{ color: '#9CA3AF' }}>No hay casos.</p> : sorted.map(c => {
        ...
        const estadoTalent = c.estado_talent || 'Pendiente'
        return (
          <div key={c.id} style={{ background: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', opacity: isCerrado ? 0.6 : 1, borderLeft: `4px solid ${bg}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
              ...
              <div>
                {c.area === 'Admin+Talent' && <SectorBadge label="Admin" estado={c.estado_admin || 'Pendiente'} />}
                <SectorBadge label="Talent" estado={estadoTalent} />
              </div>
            </div>
